...
use App\Jobs\SwooleTestTaskJob;
...
/***
 * 在 Laravel 框架中使用协程 —— 创建一个用户自定义的进程
 * Class TestProcess
...
class TestProcess implements CustomProcessInterface
{
    ...
    public static function callback(Server $swoole, Process $process)
    {
        ...
        while (true) {  // 需要通过 while(true) 进行循环保持不退出
            ...
            // task_ipc_mode 用于设置 task worker 与 worker 进程之间的通信模式，
            // 1 表示通过 unix socket，2 表示使用消息队列
            $ret = Task::deliver(new SwooleTestTaskJob('process task data'), true);
            var_dump($ret);
            // 上一层会捕获本函数抛出的异常并将其记录到 Swoole 日志中，如果异常数超过10个，该进程会退出，
            // 然后 Master 进程会重新创建这个进程
        }
    }
